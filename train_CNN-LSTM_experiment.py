...
class CNNLSTM(nn.Module):
    def __init__(self, cnn_input_size, lstm_input_size, hidden_size, num_layers,
        ...
        self.lstm = LSTMModel(input_size = lstm_input_size, hidden_size = hidden_size,
                              ...
                              dropout_p=dropout_p)
        # create flag to have seq lens only print once
        self.seq_lens_printed = False
    def forward(self, x, seq_lens):
...
for epoch in range(num_epochs):
    ...
    for i, (inputs,labels, _, _, _, seq_lens) in enumerate(trainloader):
        ...
        batch_mw_count = (labels == 1).sum().item()
        batch_not_mw_count = (labels == 0).sum().item()
        """
        ...
        if i % 100 == 0:
            ...
            print("Epoch: %d Minibatch %5d loss: %.5f" %(epoch +1, i+1, loss.item()))
            #print(f"Alpha 1-p_mw: {alpha}, Gamma: {gamma}")
            #print(f"Pos weight value: not_mw_count/mw_count: {batch_not_mw_count/batch_mw_count}")
